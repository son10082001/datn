...
function wrapExpressRequestHandler(
  ...
  if (instrumentServer.isRequestHandlerWrapped) {
  ...
  return async function (
    ...
    if (!pkg) {
      try {
        ...
        if (!pkg) {
          (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && utils.logger.error('Could not find `react-router-dom` package.');
        }
      }
    }
    ...
    if (!options || !tracing.hasTracingEnabled(options) || !request.url || !request.method) {
...
function wrapExpressCreateRequestHandler(
  ...
  return function ( options) {
    const newBuild = instrumentServer.instrumentBuild((options ).build);
