...
import { logger, extractRequestData, isString } from '@sentry/utils';
import { cwd } from 'process';
import { createRoutes, isRequestHandlerWrapped, getTransactionName, startRequestHandlerTransaction, instrumentBuild } from '../instrumentServer.js';
  ...
  return async function (
    ...
    if (!pkg) {
      try {
        ...
        if (!pkg) {
          (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error('Could not find `react-router-dom` package.');
        }
      }
    }
...
function wrapExpressCreateRequestHandler(
  ...
  return function ( options) {
    const newBuild = instrumentBuild((options ).build);
